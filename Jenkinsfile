        ...
        stage('Deploy') {
            ...
            steps {
                sshagent(credentials: ['AppServer']) {
                    ...
                    sh """ssh -t -o StrictHostKeyChecking=no ubuntu@172.31.49.124 'cd recipe/app && chmod 755 deploy.sh && ./deploy.sh develop ${CONTAINER_REGISTRY} ${env.CONTAINER_REPO}:${env.BUILD_ID}'"""
                }
            }
        }
    }
